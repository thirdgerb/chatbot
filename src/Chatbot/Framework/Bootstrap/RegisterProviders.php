...
class RegisterProviders implements Bootstrapper
{
    ...
    public function bootstrap(Application $app) : void
    {
        ...
        $app->getReactorLogger()->debug("load reactor providers : ");
        foreach ($config->reactorProviders as $providerName) {
            $app->getReactorLogger()->debug("load reactor provider $providerName");
            $app->registerReactorService($providerName);
        }

        // 注册conversation 的服务, 同一个reactor 进程内每个请求相隔离.
        $app->getReactorLogger()->debug("load conversation providers : ");
        foreach ($config->conversationProviders as $providerName) {
            $app->getReactorLogger()->debug("load conversation provider $providerName");
